...
Use snake_case for functions and variables - much easier to read than camelCase, specially for long names. The ecosystem already handles mixed conventions since Vyper contracts use snake_case.
...
- Modifiers separated by double spaces for security visibility: `external  payable  onlyOwner`

```solidity
function _calculate_fee( uint256 amount ) private returns ( uint256 )
