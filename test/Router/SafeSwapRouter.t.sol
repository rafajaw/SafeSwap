...
contract SafeSwapRouterTest is ISafeSwapRouterTests, SafeSwapRealEnv {
    ...
    function _create_position( TestERC20 token_a, TestERC20 token_b ) internal
    {
        CreatePositionParams memory params  =  CreatePositionParams({
            ...
            maximum_deposit_a: TokenAmount({ token: IERC20(address(token_a)), amount: 1_000_000 ether }),
            minimum_deposit_a: 0,
            maximum_deposit_b: TokenAmount({ token: IERC20(address(token_b)), amount: 1_000_000 ether }),
            minimum_deposit_b: 0
        });
