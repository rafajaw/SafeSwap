...
contract BondRouteIntegrationTest is SafeSwapTestBase {
    ...
    function test_quote_call_add_liquidity_returns_correct_min_stake( ) external view
    {
        ...
        // Liquidity stake is 2% of fundings[0] amount (token0).
        assertEq(
            constraints.min_stake.amount,
            2 ether,
            "Min stake should be 2% of token0 funding amount."
        );
        assertEq(
            address(constraints.min_stake.token),
            address(token0),
            "Stake token should be token0 from fundings."
        );
    }

    function test_quote_call_add_liquidity_returns_two_fundings( ) external view
    {
        AddLiquidityParams memory params  =  _create_add_liquidity_params( );
        bytes memory call_data  =  _encode_add_liquidity_calldata( params );
        ...
        assertEq(
            constraints.min_fundings.length,
            2,
            "Add liquidity should require 2 fundings."
        );
        assertEq(
            address(constraints.min_fundings[ 0 ].token),
            address(token0),
            "First funding token should be token0."
        );
        assertEq(
            constraints.min_fundings[ 0 ].amount,
            100 ether,
            "First funding amount should match preferred_fundings."
        );
        assertEq(
            address(constraints.min_fundings[ 1 ].token),
            address(token1),
            "Second funding token should be token1."
        );
        assertEq(
            constraints.min_fundings[ 1 ].amount,
            200 ether,
            "Second funding amount should match preferred_fundings."
        );
    }
    ...
    function test_quote_call_add_liquidity_returns_correct_execution_delays( ) external view
    {
        ...
        assertEq(
            constraints.max_execution_delay_in_seconds,
            4 hours,
            "Max execution delay for liquidity should be 4 hours."
        );
    }
    ...
    function test_quote_call_remove_liquidity_returns_correct_min_stake( ) external view
    {
        ...
        // Liquidity stake is 2% of amount0_min.
        assertEq(
            constraints.min_stake.amount,
            1 ether,
            "Min stake should be 2% of amount0_min (50 ether)."
        );
        assertEq(
            address(constraints.min_stake.token),
            address(token0),
            "Stake token should be token0."
        );
    }

    function test_quote_call_remove_liquidity_returns_zero_fundings( ) external view
    {
        ...
        assertEq(
            constraints.min_fundings.length,
            0,
            "Remove liquidity should require 0 fundings."
        );
    }
