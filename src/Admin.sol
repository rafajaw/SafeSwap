...
/**
 * @title Admin
 * @notice Admin functions - fee withdrawal and collector management (2-step transfer)
 */
abstract contract Admin is User {
    ...
    external  returns ( uint256 withdraw_amount )
    {
        ...
        {
            bool success  =  token.transfer( recipient, withdraw_amount );
            if(  success == false  )  revert TransferFailed({ token: address(token), recipient: recipient, amount: withdraw_amount });
        }
