...
library DonateLib {
    ...
    function get_constraints( DonateParams memory params, TokenAmount[] memory preferred_fundings )
    internal pure returns ( BondConstraints memory constraints )
    {
        ...
        uint256 amount0  =  preferred_fundings[ 0 ].amount;
        IERC20 token1    =  preferred_fundings[ 1 ].token;
        uint256 amount1  =  preferred_fundings[ 1 ].amount;

        if(  address(token0) != address(params.token0) || address(token1) != address(params.token1)  )  revert( "Fundings must match donation tokens" );

        constraints.min_stake  =  amount0 > 0
            ? SafeSwapCommon.calculate_liquidity_stake( token0, amount0 )
            : SafeSwapCommon.calculate_liquidity_stake( token1, amount1 );
