 ...
 *         from the clone's own address, so `get_config` reverts when called on the implementation rather than a clone.
 */
interface ISafeSwapHook {
    function get_config( ) external view returns ( uint16 base_fee_bps, uint8 rebate_percent );
